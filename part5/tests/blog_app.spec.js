...
test.describe('Blog app', () => {
  ...
  test.describe('When logged in', () => {
    ...
    test.describe('and blogs exist', () => {
      test.beforeEach(async ({ page, request }) => {
        const loginRes = await request.post(`${apiUrl}/api/login`, { data: { username: 'testuser', password: 'secret' } })
        const { token } = await loginRes.json()
        await createBlog(request, token, { title: 'Existing Blog', author: 'Author', url: 'http://example.com', likes: 10 })
        await createBlog(request, token, { title: 'Second Blog', author: 'Another', url: 'http://another.com', likes: 50 })
        await createBlog(request, token, { title: 'Most Liked', author: 'Top', url: 'http://top.com', likes: 100 })
        await page.goto(baseUrl)
      })
